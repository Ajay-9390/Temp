...
import { usePOTrace } from "@/hooks/useAttainmentTrace";
...
export default function PODetailPage({ params }: Props) {
  const { id } = use(params);
  const { data: trace, isLoading, isError, error } = usePOTrace(id);
  ...
  if (isError) {
    return (
      <Alert variant="destructive" role="alert">
        ...
        <AlertDescription>
          {apiError?.message ?? "Failed to load PO attainment details."}
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="space-y-6" data-testid="po-detail-page">
      <div className="flex items-center gap-3">
        ...
        >
          <ArrowLeft className="h-4 w-4" aria-hidden /> Back to PO List
        </Link>
      </div>
        ...
        <CardHeader>
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <CardTitle className="text-xl mt-1">{trace.poCode}</CardTitle>
              <p className="text-sm text-muted-foreground mt-1 max-w-2xl">{trace.poDescription}</p>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground">
            Expand each course to see individual CO contributions
          </p>
        </CardHeader>
        <CardContent className="space-y-3">
          {trace.courseBreakdown.length === 0 ? (
            ...
            trace.courseBreakdown.map((cb) => (
              <CourseSection key={cb.courseId} course={cb} />
            ))
          )}
        </CardContent>
      </Card>
